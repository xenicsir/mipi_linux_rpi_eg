#!/usr/bin/env python3
"""microlynxCtrl.py — Userspace GenCP register access for Microlynx MIPI cameras.

The microlynx kernel driver exposes a chardev at /dev/microlynx-<bus>-<addr>
(e.g. /dev/microlynx-9-0051).  This script uses ioctl on that device to call
GENCPCLIENT_ReadRegister, GENCPCLIENT_WriteRegister and GENCPCLIENT_ReadString
from userspace without any I2C_SLAVE_FORCE tricks.

Usage:
    microlynxCtrl.py [DEVICE] COMMAND [ARGS...]

DEVICE (optional):
    Path to the GenCP chardev.
    Default: first /dev/microlynx-* device found automatically.
...
Commands:
    ...
    read_string  ADDR [LENGTH]    Read a GenCP string register (default LENGTH=64)
...
Examples:
    ...
    microlynxCtrl.py read_reg32 0x50FF0010

    # Explicit device path
    microlynxCtrl.py /dev/microlynx-9-0051 read_reg32 0x50FF0000

    # Read firmware version as hex
    microlynxCtrl.py read_reg32 0x50FF0000

    # Read image height as float (should print integer-valued float)
    microlynxCtrl.py read_reg32f 0x500E000C

    # Start acquisition
    microlynxCtrl.py write_reg32 0x500F0000 1

    # Read model name string
    microlynxCtrl.py read_string 0x00000044

    # Read serial number
    microlynxCtrl.py read_string 0x00000144
"""
...
# ---------------------------------------------------------------------------
# IOCTL constants  (must match kernel definitions in microlynx_core.c)
#
...
#   type: 'M' = 0x4D
#
#   struct microlynx_reg_op { u32 addr; u32 val; }   → sizeof = 8
#   struct microlynx_str_op { u32 addr; u32 len; u8 buf[256]; } → sizeof = 264
#
#   MICROLYNX_IOCTL_READ_REG  = _IOWR('M', 1, 8)   = 0xC0084D01
#   MICROLYNX_IOCTL_WRITE_REG = _IOW ('M', 2, 8)   = 0x40084D02
#   MICROLYNX_IOCTL_READ_STR  = _IOWR('M', 3, 264) = 0xC1084D03
# ---------------------------------------------------------------------------
...
MICROLYNX_STR_MAX = 256

MICROLYNX_IOCTL_READ_REG  = _IOWR('M', 1, 8)
MICROLYNX_IOCTL_WRITE_REG = _IOW ('M', 2, 8)
MICROLYNX_IOCTL_READ_STR  = _IOWR('M', 3, 4 + 4 + MICROLYNX_STR_MAX)

# struct formats (little-endian, matching kernel __u32 / __u8)
_REG_FMT = '<II'                        # addr, val
_STR_FMT = f'<II{MICROLYNX_STR_MAX}s'  # addr, len, buf
...
def _auto_detect():
    devices = sorted(glob.glob('/dev/microlynx-*'))
    if not devices:
        raise FileNotFoundError(
            "No /dev/microlynx-* chardev found.\n"
            "Make sure the microlynx kernel module is loaded and the camera "
            "has been probed successfully (check: dmesg | grep microlynx)."
        )
    return devices[0]
...
def _ioctl_rw(dev_path, ioctl_code, fmt, *pack_args):
    ...
    with open(dev_path, 'rb+', buffering=0) as f:
        fcntl.ioctl(f.fileno(), ioctl_code, buf)  # returns int; buf is mutated
...
def read_reg32(dev_path, addr):
    """Read a 32-bit GenCP register.  Returns the integer value."""
    _, val = _ioctl_rw(dev_path, MICROLYNX_IOCTL_READ_REG, _REG_FMT, addr, 0)
    return val


def read_reg32f(dev_path, addr):
    """Read a 32-bit GenCP register and interpret as IEEE 754 float."""
    val = read_reg32(dev_path, addr)
    return struct.unpack('<f', struct.pack('<I', val))[0]


def write_reg32(dev_path, addr, val):
    """Write a 32-bit integer value to a GenCP register."""
    _ioctl_w(dev_path, MICROLYNX_IOCTL_WRITE_REG, _REG_FMT, addr, val & 0xFFFFFFFF)


def write_reg32f(dev_path, addr, fval):
    """Write a float value (IEEE 754) to a GenCP register."""
    ival = struct.unpack('<I', struct.pack('<f', float(fval)))[0]
    write_reg32(dev_path, addr, ival)


def read_string(dev_path, addr, length=64):
    """Read a GenCP string register.  Returns a decoded ASCII string."""
    length = min(int(length), MICROLYNX_STR_MAX)
    _, _, raw = _ioctl_rw(
        dev_path, MICROLYNX_IOCTL_READ_STR, _STR_FMT,
        addr, length, b'\x00' * MICROLYNX_STR_MAX
    )
    # Strip padding: kernel GENCPCLIENT_ReadString may fill unused bytes with
    # 0xff (GenCP "unused" marker) or leave them as 0x00.
...
def main():
    ...
    if args and (args[0].startswith('/dev/') or args[0].startswith('microlynx')):
        dev_path = args[0]
        args = args[1:]
    else:
        try:
