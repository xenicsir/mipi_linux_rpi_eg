...
static const struct tc358746_mbus_fmt tc358746_formats[] = {
   {
      ...
      /*
       * Same bridge configuration as RGB888_1X24 above, byte for byte: the
       * parallel side is identical and only the label differs. The entry
       * exists because the Raspberry Pi driver advertises BGR888_1X24 to the
       * RP1 CFE, which is what makes the capture come out as BGR3; the Jetson
       * driver maps everything to RGB888_1X24 instead, so its copy of this
       * file has no such entry. Dropping it makes tc358746_calculate() return
       * -EINVAL for every link frequency, which looks like a link frequency
